...
function sanitizeModeName(name, idx) {
  const fallback = DEFAULT_MODE_NAMES[idx] || `Mode ${idx + 1}`;
  if (typeof name !== 'string') return fallback;
  const trimmed = name.trim();
  return trimmed ? trimmed.slice(0, MODE_NAME_MAX_LENGTH) : fallback;
}

function normalizeModeStore(store) {
  const normalized = createDefaultModeStore();
  if (!store || typeof store !== 'object') return normalized;

  const active = Number(store.activeMode);
  normalized.activeMode = Number.isInteger(active) && active >= 0 && active < MODE_COUNT ? active : 0;

  const rawModes = Array.isArray(store.modes) ? store.modes : [];
  for (let i = 0; i < MODE_COUNT; i++) {
    const mode = rawModes[i];
    if (!mode || typeof mode !== 'object') continue;
    normalized.modes[i] = {
      name: sanitizeModeName(mode.name, i),
      snapshot: mode.snapshot && typeof mode.snapshot === 'object' ? clonePlainValue(mode.snapshot) : {}
    };
  }
  return normalized;
}
...
function saveState() {
  if (!modeStore.modes[modeStore.activeMode]) modeStore = normalizeModeStore(modeStore);
...
function loadState() {
  try {
    ...
    if (rawV2) {
      ...
      if (rawV1) {
        const legacySnap = JSON.parse(rawV1);
        if (legacySnap && typeof legacySnap === 'object') {
          modeStore.modes[0].snapshot = clonePlainValue(legacySnap);
        }
      }
      persistModeStore();
    }
...
function renameMode(idx) {
  ...
  modeStore.modes[idx].name = sanitizeModeName(input, idx);
  persistModeStore();
  renderModeTabs();
}
...
function renderActiveMode() {
  ...
  if (!state.manualPlacement) hideManualPicker();
}
